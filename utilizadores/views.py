...
from .models import Utilizador, Cliente, Funcionario
...
def apagar_utilizador(request, id): 
    ...
    if user.groups.filter(name = "Funcionario").exists():
        ...
        for inscricao in Inscricao.objects.filter(Cliente=u):
            ...
            for inscricaosessao in inscricaosessao_set:
                ...
                nClientes = inscricaosessao.nClientes
                with transaction.atomic():
                    sessao = Sessao.objects.select_for_update().get(pk=sessaoid)
                    sessao.vagas = F('vagas') + nClientes
                    sessao.save()
            inscricao.delete()
    else:
        u = user    
...
def apagar_proprio_utilizador(request):  
    ...
    if request.user.is_authenticated:
        ...
        if user.groups.filter(name = "Coordenador").exists():
            ...
            try:
                u = Cliente.objects.filter(id=id) 
                u = Cliente.objects.get(id=id)
                for inscricao in Inscricao.objects.filter(Cliente=u):
                    inscricaosessao_set = inscricao.inscricaosessao_set.all()
                    for inscricaosessao in inscricaosessao_set:
...
def alterar_utilizador_admin(request,id):
    ...
    if user.groups.filter(name = "Coordenador").exists():
        ...
        tipo=1
        u = "Cliente" 
        utilizador_object = Cliente.objects.get(id=user.id)
        utilizador_form = ClienteAlterarPerfilForm(instance=utilizador_object)
        perfil= "Cliente"
    else:
        return redirect('utilizadores:mensagem',5)     
    ...
    if request.method == "POST":
        ...
        if tipo == 1:
            form = ClienteAlterarPerfilForm(submitted_data,instance=utilizador_object)
            my_group = Group.objects.get(name='Cliente') 
        elif tipo == 2:
...
def mudar_perfil_escolha(request):
    ...
    utilizadores = ["Cliente", "Funcionario","Administrador"]
...
def mudar_perfil_admin(request,tipo,id):
    ...
    if tipo == 1:
        form = ClienteAlterarPerfilForm()
        perfil = "Cliente"
    elif tipo == 2:
        form = FuncionarioAlterarPerfilForm()
        perfil = "Funcionario"
    elif tipo == 3:
    ...
    if request.method == "POST":
        ...
        if tipo == 1:
            form = ClienteAlterarPerfilForm(submitted_data)
            my_group = Group.objects.get(name='Cliente')
        elif tipo == 2:
            form = FuncionarioAlterarPerfilForm(submitted_data)
            my_group = Group.objects.get(name='Funcionario')
        elif tipo == 3:
